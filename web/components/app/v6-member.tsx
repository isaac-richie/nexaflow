  ...
  return <div className={styles.stack}>
    ...
    {!IS_DEPLOYED ? <NotDeployedNotice /> : !isConnected ? <ConnectPrompt /> : chainId !== ACTIVE_CHAIN.id ? <section className={styles.notice}><h2>Switch to {ACTIVE_CHAIN.name}</h2><p>Your membership and payments must use the correct network.</p><ConnectButton /></section>
      ...
      : <>
        ...
        <p className={styles.muted}>Use Refresh for the latest updates. Earnings and board completion are not guaranteed.</p>
      </>}
  </div>;
}
...
function TransactionStatus({ tx }: { tx: ReturnType<typeof useV6Transaction> }) {
  return <div aria-live="polite" className="space-y-3 text-sm">
    ...
    {tx.queueStage !== undefined && <section className={styles.notice} aria-label="Prepare stage for entry">
      <p>A separate update can help prepare Stage {tx.queueStage + 1}. It costs BNB network fees, but no additional USDT entry fee. It does not register you or guarantee a placement.</p>
      <button className={`${styles.button} mt-3 min-h-11 w-full sm:w-auto`} disabled={tx.busy} onClick={() => void tx.execute("process", { stage: tx.queueStage })}>{tx.busy ? "Checking transaction…" : `Prepare Stage ${tx.queueStage + 1} · BNB gas required`}</button>
      <p className="mt-2">You can also wait and retry Join later. Never send funds manually.</p>
    </section>}
...
function ClaimPayout({ amount }: { amount: bigint }) {
  ...
  return <section className={styles.section}><h2>Claim an unpaid payout</h2><p className={styles.muted}>Claim unpaid rewards—not re-entry funds. Leave the recipient blank to use your wallet.</p>
...
function V6Join({ snapshot: s, member }: { snapshot: V6Snapshot; member: Address }) {
  ...
  return <div className={styles.stack}>
    <section className={styles.section}>
      <h2>{config ? `Join Stage ${stage + 1}` : "All six stages unlocked"}</h2><p className={styles.muted}>Start at Stage 1, then unlock stages in order. No board completion needed to upgrade.</p>
      ...
      {!s.registered && <div className="mb-5"><label htmlFor="v6-sponsor" className="block text-sm">Sponsor address (optional)</label><input id="v6-sponsor" className="my-3 w-full rounded-xl border border-line bg-surface-2 p-3 text-sm" value={sponsor} onChange={e => changeSponsor(e.target.value.trim())} disabled={tx.busy} placeholder="Leave blank to start under the protocol" autoComplete="off" spellCheck={false} />
        <p className={styles.muted}>Use a current member’s address, or leave blank to join under the protocol.</p>
      ...
      {config && <div className={styles.stack}>
        ...
        <p className={styles.muted}>Your reserve funds the next board—not a cash bonus. <Link href="/legal#v6-reserves" className="underline underline-offset-2">Learn more</Link></p>
...
function BoardDetails({ stage, snapshot, member, client }: { stage: V6Stage; snapshot: V6Snapshot; member: Address; client: PublicClient }) {
  ...
  return <div className={styles.stack}>
    <div className={styles.boardLayout}><div><span className={styles.badge}>Board #{p.cycle.toString()} · {p.queued ? "Re-entry queued" : p.closed ? "Completed" : "Open"}</span>
      ...
      <p className={styles.muted}>Board placements—not your referral tree. Green: open. Brown: completed.</p>
    ...
    <p className={styles.muted}>{snapshot.recovery ? "Reserve amounts are records, not confirmation of funds currently held by the contract during recovery." : <>Your reserve funds the next board—not a cash bonus. <Link href="/legal#v6-reserves" className="underline underline-offset-2">Learn more</Link></>}</p>
    {stage.latest?.queued && <section className={styles.notice}><p>Re-entry is pending. Purchases help process the queue. Manual processing costs BNB; placement may still be delayed.</p><button className={styles.button} disabled={tx.busy || snapshot.recovery} onClick={() => void tx.execute("process", { stage: stage.id })}>Process pending re-entries</button><TransactionStatus tx={tx} /></section>}
