...
function render(view = "overview", options = {}) {
  ...
  const module = loadV6("components/app/v6-member.tsx", {
    ...
    "@/lib/v6": model,
    "@/hooks/use-v6": { useV6Snapshot: () => ({ account, data: s, client: {}, isPending: options.loading, isError: options.error, error: options.error ? new Error("Contract identity check failed") : undefined }), useV6Transaction: () => ({ busy: false, ...options.tx }), v6Error: e => e.message, v6Scope: [], v6Deployment: d },
...
test("unregistered join screen exposes mobile-sized explicit queue recovery with gas disclosure", () => {
  const s = sample(); s.registered = false;
  const html = render("join", { snapshot: s, tx: { queueStage: 0 } });
  assert.match(html, /Prepare Stage 1/);
  assert.match(html, /BNB gas required/);
  assert.match(html, /no additional USDT entry fee/);
  assert.match(html, /does not register you or guarantee a placement/);
  assert.match(html, /min-h-11 w-full sm:w-auto/);
  assert.match(render("join", { snapshot: s, tx: { queueStage: 0, busy: true } }), /disabled=""[^>]*>Checking transaction/);
  assert.doesNotMatch(render("join", { snapshot: s }), /BNB gas required/);
});
