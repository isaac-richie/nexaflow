...
function fixture(options = {}) {
  ...
  const client = {
    readContract: async request => { reads.push(request); if (request.functionName === "queueLength") { if (options.queueReadError) throw new Error("RPC unavailable"); return options.queue ?? 0n; } return request.functionName === "allowance" ? fee : options.unknownSponsor ? false : true; },
    simulateContract: async request => { simulations.push(request); if (options.simulationError && request.functionName !== "processReentries") throw new Error(options.simulationError === true ? "PlacementUnavailable" : options.simulationError); return { request, result: options.processed ?? 1n }; },
...
function processLogs(stage = 0) {
  return [{ address: membership,
    topics: encodeEventTopics({ abi, eventName: "PositionOpened", args: { id: 9n, member: token, stage } }),
    data: encodeAbiParameters([{ type: "uint256" }, { type: "uint64" }, { type: "bool" }], [1n, 2n, true]),
  }, { address: membership,
    topics: encodeEventTopics({ abi, eventName: "Reactivated", args: { oldPosition: 7n, newPosition: 9n } }),
    data: encodeAbiParameters([{ type: "uint256" }], [fee]),
  }];
}
test("blocked registration exposes opt-in queue recovery without submitting anything", async () => {
  const f = fixture({ simulationError: true, queue: 1n });
  await f.hook.execute("purchase", { stage: 0, fee });
  assert.equal(f.writes.length, 0); assert.equal(f.result().queueStage, 0);
  assert.match(f.result().error, /No purchase was submitted/);
});
test("blocked upgrades recover the selected stage without changing sponsor or stage order", async () => {
  const f = fixture({ simulationError: true, queue: 1n, snapshot: { registered: true, highestStage: 2, stages: [{ fee }, { fee }, { fee }, { fee }] } });
  await f.hook.execute("purchase", { stage: 3, fee });
  assert.equal(f.result().queueStage, 3); assert.equal(f.writes.length, 0);
  assert.equal(f.simulations[0].functionName, "joinStage");
  assert.equal(f.reads.at(-1).args[0], 3n);
});
test("non-placement errors, empty queues and failed queue reads do not offer recovery", async () => {
  for (const options of [{ simulationError: "StageNotAvailable", queue: 1n }, { simulationError: true, queue: 0n }, { simulationError: true, queueReadError: true }]) {
    const f = fixture(options); await f.hook.execute("purchase", { stage: 0, fee });
    assert.equal(f.result().queueStage, undefined); assert.equal(f.writes.length, 0);
  }
});
test("unregistered wallet can explicitly process any stage without USDT or approval", async () => {
  for (let stage = 0; stage < 6; stage++) {
    const f = fixture({ queue: 1n, snapshot: { balance: 0n, allowance: 0n }, logs: processLogs(stage) });
    await f.hook.execute("process", { stage });
    assert.equal(f.writes.length, 1); assert.equal(f.writes[0].functionName, "processReentries");
    assert.deepEqual(Array.from(f.writes[0].args), [stage, 16n]);
    assert.match(f.result().message, /did not purchase a stage/);
    assert.equal(f.result().error, undefined);
  }
});
test("recovery requires a separate click and never automatically registers after processing", async () => {
  const options = { simulationError: true, queue: 1n, logs: processLogs() };
  const f = fixture(options); await f.hook.execute("purchase", { stage: 0, fee });
  assert.equal(f.writes.length, 0);
  await f.hook.execute("process", { stage: 0 });
  assert.equal(f.writes.length, 1); assert.equal(f.writes[0].functionName, "processReentries");
  assert.equal(f.result().queueStage, undefined);
  assert.match(f.result().message, /Select Join/);
});
test("empty or non-progressing queues never request a gas-paying transaction", async () => {
  const empty = fixture({ queue: 0n }); await empty.hook.execute("process", { stage: 0 });
  assert.equal(empty.writes.length, 0); assert.match(empty.result().message, /No re-entry work remains/);
  const stuck = fixture({ queue: 1n, processed: 0n }); await stuck.hook.execute("process", { stage: 0 });
  assert.equal(stuck.writes.length, 0); assert.match(stuck.result().error, /No re-entry can be processed/);
});
test("processing blocks recovery mode, invalid stages and changed wallets", async () => {
  for (const options of [{ snapshot: { recovery: true } }, { walletChain: 97 }, { walletAddress: token }]) {
    const f = fixture({ ...options, queue: 1n }); await f.hook.execute("process", { stage: 0 });
    assert.equal(f.writes.length, 0); assert.ok(f.result().error);
  }
  for (const stage of [-1, 6, 0.5, NaN]) {
    const f = fixture({ queue: 1n }); await f.hook.execute("process", { stage }); assert.equal(f.writes.length, 0);
  }
});
test("reverted, unrelated, no-op or cancelled processing receipts never report progress", async () => {
  for (const options of [{ logs: [] }, { logs: processLogs(1) }, { logs: processLogs(), receipt: { status: "reverted" } }, { logs: processLogs(), replaced: "cancelled" }]) {
    const f = fixture({ ...options, queue: 1n }); await f.hook.execute("process", { stage: 0 });
    assert.ok(f.result().error); assert.equal(f.result().message, undefined);
  }
});
