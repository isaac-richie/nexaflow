...
export function useV6Transaction() {
  ...
  const [queueStage, setQueueStage] = useState<number>();
  ...
  async function execute(action: "approve" | "purchase" | "claim" | "process", options: { stage?: number; fee?: bigint; sponsor?: Address; recipient?: Address } = {}) {
    ...
    if (action === "purchase" || action === "approve") setQueueStage(undefined);
    try {
      ...
      if (action === "approve") {
        ...
        // A purchase can roll queue progress back when its own placement fails.
        // Commit queue work separately, but only on an explicit user action.
        // Never automatically sign maintenance or retry a purchase.
        const simulatePurchase = () => fresh.registered
          ? client.simulateContract({ address: MEMBERSHIP_ADDRESS, abi, functionName: "joinStage", args: [stage], account: address })
          : client.simulateContract({ address: MEMBERSHIP_ADDRESS, abi, functionName: "register", args: [sponsor], account: address });
        let simulated;
        try { simulated = await simulatePurchase(); }
        catch (failure) {
          if (/PlacementUnavailable/.test(failure instanceof Error ? failure.message : String(failure))) {
            const pending = await client.readContract({ address: MEMBERSHIP_ADDRESS, abi, functionName: "queueLength", args: [BigInt(stage)] });
            if (pending > 0n) {
              setQueueStage(stage);
              throw new Error("This stage needs a re-entry update before you can join. No purchase was submitted. You can prepare the stage below, or wait for it to be processed.");
            }
          }
          throw failure;
        }
        ...
        if (!Number.isInteger(stage) || stage < 0 || stage > 5) throw new Error("Choose a valid stage to prepare.");
        // Global stage queue: new members must be able to unblock registration,
        // even though they do not own a board yet. Do not require USDT allowance.
        const pending = await client.readContract({ address: MEMBERSHIP_ADDRESS, abi, functionName: "queueLength", args: [BigInt(stage)] });
        if (pending === 0n) {
          setQueueStage(undefined);
          setMessage("No re-entry work remains for this stage. You can select Join to check your entry again.");
          return;
        }
        const simulated = await client.simulateContract({ address: MEMBERSHIP_ADDRESS, abi, functionName: "processReentries", args: [stage, 16n], account: address });
        if (simulated.result === 0n) throw new Error("No re-entry can be processed right now. No transaction was submitted. Please wait or contact support; do not keep paying network fees.");
        submitted = await send(simulated.request);
      }
      ...
      if (action === "process") {
        const events = receipt.logs.flatMap(log => {
          if (log.address.toLowerCase() !== MEMBERSHIP_ADDRESS.toLowerCase()) return [];
          try { return [decodeEventLog({ abi, data: log.data, topics: log.topics })]; } catch { return []; }
        });
        if (!events.some(event => event.eventName === "Reactivated") || !events.some(event => event.eventName === "PositionOpened" && event.args.stage === stage)) throw new Error("Receipt did not confirm re-entry progress for this stage. Check the transaction before trying again.");
        setQueueStage(undefined);
      }
      ...
      setMessage(action === "approve" ? "Approval confirmed. Now select Join to complete your registration." : action === "purchase" ? `Stage ${stage + 1} joined successfully. View your board below or on My boards.` : action === "claim" ? "Payout claimed successfully." : "Re-entry progress confirmed. This did not purchase a stage. Select Join to check your entry again, or Refresh to view your board. More queue work may still be needed.");
    } catch (failure) { setError(v6Error(failure)); setMessage(undefined); }
    finally {
      try { await cache.invalidateQueries({ queryKey: v6Scope }); }
      finally { lock.current = false; setBusy(false); }
    }
  }
  return { execute, busy, message, error, hash, queueStage };
}
